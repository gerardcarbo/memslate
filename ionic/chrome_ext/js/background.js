...
app.run(function ($q, SessionService, TranslationsProviders, TranslateService) {
  ...
  function onTranslationResponse(provider, sl, tl, word, data, sendResponse) {
    ...
    if (provider === "Yandex") {
      ...
      if (data.rawResult.translatedText) {
        var definition = { pos: '', meanings: [], syn: [] };
        definition.meanings.push(data.rawResult.translatedText);
        output.push(definition);
      }
    }
  ...
  function translateWithService(sl, tl, request, sendResponse) {
    ...
    return TranslateService.translate(sl, tl, request.word)
      .then(function (translation) {
        if (translation.rawResult.translatedText.toLowerCase() == request.word.toLowerCase()) {
          //no translation found. Translate sample
          return TranslateService.translate(sl, tl, request.sample).success(function (translation) {
