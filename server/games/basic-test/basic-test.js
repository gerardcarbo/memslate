...
module.exports = function (bookshelf, models) {
    ...
    function get(req,res) {
        ...
        if(req.params.op === 'languages')
        {
            ...
            bookshelf.knex.raw('SELECT t."fromLang", t."toLang", count(*) as count FROM public."Translations" as t '+
                                ...
                                'WHERE ut."userId"='+req.user.id+' GROUP BY t."fromLang", t."toLang" '+
        ...
        {
            return getQuestions(req, req.query.fromLang, req.query.toLang, req.query.questions)
                .then(function(questions) {
                    var questionTranslations=questions.rows;
                    getQuestions(req, req.query.fromLang, req.query.toLang, req.query.questions*(req.query.answers))
                        .then(function(answers) {
                            var questionAnswers = answers.rows;
                            var guestionsData = [];
                            questionTranslations.forEach(function(question){
                                var questionData={question:question.translate, answer:question.mainResult, options:[]};
    ...
    function getQuestions(req, fromLang, toLang, number)
    {
        return bookshelf.knex.raw('SELECT * FROM public."Translations" as t INNER JOIN public."UserTranslations" as ut ON t.id=ut."translationId" WHERE ut."userId"='+req.user.id+
